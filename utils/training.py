...
def batch_resize(img_batch, target_shape=(224, 224)):
    """(bsz, 3, original_shape) -> (bsz, 3, target_shape)"""
    resize = Resize(target_shape, antialias=True)
    transformed_images = []
...
def batch_visualization(img_batch, img_name='vis', count=3):
    i = 0
    for img in img_batch:
        if i <= count:
            plt.imshow(np.transpose(img, (1,2,0)))
            plt.savefig(
                f'/home/ubuntu/RES-main/SAMC+BayesPCN/visualization/{img_name}_{i}.png')
            i += 1
...
def evaluate(model: ContinualModel, dataset: ContinualDataset, last=False, eval_ema=False, ema_model=None) -> Tuple[list, list]:
    ...
    for k, test_loader in enumerate(dataset.test_loaders):
        ...
        for data in test_loader:
            inputs, labels = data
            if 'bpcn' or 'baseline' or 'joint' or 'sgd' in model.NAME:  # which means backbone needs standard input shape (224by224)
                inputs = batch_resize(inputs)  # torch.Size([10, 3, 32, 32]) -> torch.Size([10, 3, 224, 224])
...
def save_task_perf(savepath, results, n_tasks):
    ...
    np.savetxt(savepath, results_array, fmt='%.2f', delimiter=",")
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    if args.csv_log:
        csv_logger = CsvLogger(dataset.SETTING, dataset.NAME, model.NAME, args.output_dir, args.experiment_id, args.buffer_size)
        task_perf_path = os.path.join(args.output_dir, "results", dataset.SETTING, dataset.NAME, model.NAME, f'{args.buffer_size}_{args.theta}_{args.seed}.csv')
        ...
        for ema_model in lst_ema_models:
            if hasattr(model, ema_model):
                ...
                print('=' * 50)
                ema_loggers[ema_model] = CsvLogger(dataset.SETTING, dataset.NAME, model.NAME, args.output_dir, args.experiment_id, str(args.buffer_size) + f'_{ema_model}')
                ema_task_perf_paths[ema_model] = os.path.join(args.output_dir, "results", dataset.SETTING, dataset.NAME, model.NAME, args.experiment_id + f'{args.buffer_size}_{args.theta}_{args.seed}_{ema_model}', 'task_performance.txt')
