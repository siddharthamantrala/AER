...
from models.gem_bpcn import overwrite_grad
from models.gem_bpcn import store_grad
...
def sample_from_memory(mem_data, mem_label, bsz):
    """Sample data and labels from past tasks"""
    ...
    return sampled_data, sampled_lab


class AGemBpcn(ContinualModel):
    NAME = 'agem_bpcn'
    COMPATIBILITY = ['class-il', 'task-il']

    def __init__(self, backbone, loss, args, transform):
        super(AGemBpcn, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if len(self.observed_tasks) > 1:
            # copy gradient
            store_grad(self.net.fc.parameters, self.grad_xy, self.grad_dims)
            ...
            pt_output_batch = self.forward(
                    ...
                    classifier_only=True)
            penalty = self.loss(pt_output_batch, buf_labels)
            penalty.backward()
            store_grad(self.net.fc.parameters, self.grad_er, self.grad_dims)

            dot_prod = torch.dot(self.grad_xy, self.grad_er)
            if dot_prod.item() < 0:
                g_tilde = project(gxy=self.grad_xy, ger=self.grad_er)
                overwrite_grad(self.net.fc.parameters, g_tilde, self.grad_dims)
            else:
                overwrite_grad(self.net.fc.parameters, self.grad_xy, self.grad_dims)
